...
/// Client Mistral La Plateforme. Gratuit sans carte sur console.mistral.ai.
/// API OpenAI-compatible.
class MistralService {
  static const _keyPref = 'mistral_api_key';
  static const _endpoint = 'https://api.mistral.ai/v1/chat/completions';
  static const _textModel = 'mistral-large-latest';
  static const _visionModel = 'pixtral-large-latest';
  ...
  static Future<void> init() async {
    ...
    try {
      ...
      final remote = snap.data()?['mistralApiKey'] as String?;
      if (remote != null && remote.isNotEmpty) {
        _apiKey = remote;
        await prefs.setString(_keyPref, remote);
      } else if (_apiKey != null && _apiKey!.isNotEmpty) {
        await _settingsDoc
            .set({'mistralApiKey': _apiKey}, SetOptions(merge: true));
      }
    } catch (_) {}
  }
    ...
    if (key == null || key.isEmpty) {
      ...
      try {
        await _settingsDoc.set(
            {'mistralApiKey': FieldValue.delete()},
      ...
      try {
        await _settingsDoc
            .set({'mistralApiKey': key}, SetOptions(merge: true));
      } catch (_) {}
    }
  }
  ...
  static const _jsonFormat = '''
{
  ...
  "climat": "UNIQUEMENT pour vins de Bourgogne, sinon chaîne vide",
...
Toutes les valeurs textuelles EN FRANÇAIS. marketValue = valeur marchande en CAD pour 750 ml. Si une info n'est pas connue : null pour les nombres, "" pour les textes. NE PAS INVENTER.''';
  ...
  static Future<GeminiResult> searchByText({
    ...
    if (!isConfigured) {
      throw Exception('Clé API Mistral non configurée.');
    }
    final vintageStr =
  ...
  static Future<Map<String, dynamic>> _callJson({
    ...
    if (res.statusCode != 200) {
      throw Exception('Mistral erreur ${res.statusCode} : ${res.body}');
    }

    final data = jsonDecode(res.body) as Map<String, dynamic>;
    final choices = data['choices'] as List?;
    if (choices == null || choices.isEmpty) {
      throw Exception('Mistral : réponse vide');
    }
    final content = choices.first['message']?['content'] as String?;
    if (content == null || content.isEmpty) {
      throw Exception('Mistral : contenu vide');
    }
  ...
  static Map<String, dynamic> _parseJson(String content) {
    ...
    if (start == -1 || end == -1 || end <= start) {
      throw Exception('Mistral : pas de JSON valide dans la réponse');
    }
    final jsonStr = inner.substring(start, end + 1);
    final decoded = jsonDecode(jsonStr);
    if (decoded is! Map<String, dynamic>) {
      throw Exception('Mistral : JSON inattendu');
    }
    return decoded;
  }
}
